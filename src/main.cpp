...
#if defined(NDEBUG)
# error "Amiga cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfbcbd08d41849cbe1df189d48e8044f8484d447811379c0617a7d956c301d969");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Amiga: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Amiga Signed Message:\n";
...
{
    // Amiga: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 512 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 409600000 * COIN;
            }

    // Subsidy is cut in half every 4000000 blocks
    nSubsidy >>= (nHeight / 4000000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Amiga: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Amiga: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1542839452, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Amigacoin Reborn";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 512 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0440c10c9c81e9fc6321bf741a72303287a9c954cce373bc01aad5b68df71728e0801279a7180c2553019e71fcb1795c8bdffe665f9e6b59a064bb899aca221742") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542839452;
        ...
        assert(block.hashMerkleRoot == uint256("0x9159a1bbb435301ec438084656c304db3e48c0d383c34b6ff8e4748674eb5356"));
...
unsigned char pchMessageStart[4] = { 0x85, 0x39, 0x0b, 0x4b }; // Amiga: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AmigaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AmigaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AmigaMiner(CWallet *pwallet)
{
    printf("AmigaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("amiga-miner");
    ...
    try { loop {
        ...
        printf("Running AmigaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AmigaMiner, pwallet));
}
